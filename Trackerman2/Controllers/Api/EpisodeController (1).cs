...
namespace Trackerman2.Controllers.Api
{
    public class EpisodeController : ApiController
    {
        ...
        public IHttpActionResult WatchEpisode([FromBody]int episodeID)
        {
            ...
            var user = db.Users.Include(x => x.Episodes).Include(x => x.Seasons).Single(x => x.Id == userID);
            var episode = db.Episodes.Include(x => x.Season.Episodes).Single(x => x.ID == episodeID);
